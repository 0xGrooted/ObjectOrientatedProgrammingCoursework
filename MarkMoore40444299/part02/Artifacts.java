  ...
  */
    ...
    public void setId(int id) {
        if (id <= 0) {
            ...
            System.out.println("Invalid ID provided. Assigning a new unique ID: " + this.id);
        } else {
            this.id = id;
        }
    }
    ...
    */
    ...
    public static void searchPartName(String str, ArrayList<Artifacts> artifacts) {
        ...
        if (str.equals("") || str.equals(" ")) {
            System.out.println("Invalid search criteria no spaces unless there is an actual space between names");
        } else {
            for (int i = 0; i < artifacts.size(); i++) {
            ...
            if (!found) {
                System.out.println("No artifact found with the part name");
            }
        }
    }
    ...
    public static void deleteArtifact(ArrayList<Artifacts> artifacts) {
        ...
        while (repeat) {
            try {
                System.out.print("Please enter the ID of the artifact you wish to delete: ");
                id = myObj.nextInt();
                ...
                System.out.println("Please only enter the number of an ID which exists");
                myObj.nextLine();
            }
        }
        ...
        while (repeat) {
            try {
                System.out.print("Please type 'DELETE' to delete artifact " + id + ": ");
                String deleteAttempt = myObj.next();
                ...
                if (deleteAttempt.equals("DELETE")) {
                    ...
                    System.out.println("Artifact " + id + " Removed successfully.");
                    repeat = false;
                }
            } catch (Exception E) {
    ...
    public static void addArtifact(ArrayList<Artifacts> artifacts) {
        ...
        while (Repeat) {
            System.out.print("Artifacts name: ");
            try {
        ...
        while (Repeat) {
            try {
                System.out.print("Artifacts Type: ");
                int type = myObj.nextInt();
                myObj.nextLine();

                if (type < 0 || type > 5) {
                    System.out.println("Please only select from the list above by number");
                } else {
                    type2 = ArtifactType.findFromNumber(type);
                    if (type2 == null) {
                        System.out.println("Invalid artifact type selection. Operation canceled.");
                    }
                    Repeat = false;
                }
            } catch (Exception e) {
        ...
        while (Repeat) {
            try {
                System.out.print("Enter Artifacts engagement time: ");
                engTime = myObj.nextInt();
                myObj.nextLine();

                if (engTime == 0) {
                    System.out.println("Time must be more than 0 minutes");
                } else {
                    Repeat = false;
                }
            } catch (Exception e) {
                System.out.println("Please only enter valid positive integer numbers");
                myObj.nextLine();
            }
        }
        ...
        Repeat = true;
        while (Repeat) {
            System.out.print("Please enter the artifact's description: ");
            try {
                ...
                System.out.println("Error in description, try again.");
                myObj.nextLine();
            }
        }
    ...
    public static void updateArtifact(ArrayList<Artifacts> artifacts) {
        ...
        Repeat = true;
        while (Repeat) {
            System.out.print("Enter the new name for the artifact or press enter to keep the same name: ");
            name = myObj.nextLine().trim();
            if (name.isEmpty()) {
        ...
        while (Repeat) {
            try {
                System.out.print("Enter new type (or press enter to keep existing type): ");
                String typeInput = myObj.nextLine().trim();
        ...
        Repeat = true;
        while (Repeat) {
            System.out.print("Enter the new description (or press enter to keep existing description): ");
            Description = myObj.nextLine().trim();
        ...
        while (Repeat) {
            try {
                System.out.print("Enter the new engagement time (or press enter to keep the same time): ");
                String timeInput = myObj.nextLine();
                ...
                if (timeInput.trim().isEmpty()) {
                    ...
                    if (inputTime <= 0) {
                        System.out.println("Engagement time must be positive. Please try again.");
                    }
                    else if (inputTime > 30) {
                        System.out.println("Engagement time must be less than 30 (minutes). Please try again.");
                    }else {
                        engTime = inputTime;
                        Repeat = false;
                    }
                ...
                System.out.println("Invalid input. Please enter a valid number or press Enter to keep the existing time.");
            }
        }
        ...
        System.out.println("Artifact updated successfully!");
    }
    /**
        ...
        while (RepeatLoop == true) {
            ...
            try {
                ...
                switch (selection) {
                    ...
                    case 6:
                        System.out.println("Returning to the Artifact Menu...");
